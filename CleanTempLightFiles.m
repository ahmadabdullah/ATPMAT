function[]=CleanTempLightFiles(Dir,filename )
% This is a hlper function that depetes all temp lightning files files due to ATPMAT
% operation
...
nameBase=strcat(Dir.codePath,'\',filename);
    ...
    system(fileRM{:});
    fileRM=strcat({'rm '},{nameBase},'07');
    system(fileRM{:});
    cd(Dir.ATPEXEDir);
    fileRM=strcat({'rm '},{strcat(Dir.ATPEXEDir,'\','tempLightningFile.pl4')});
    system(fileRM{:});
    fileRM=strcat({'rm '},{strcat(Dir.ATPEXEDir,'\','tempLightningFile.atp')});
    system(fileRM{:});
    fileRM=strcat({'rm '},{strcat(Dir.ATPEXEDir,'\','tempLightningFile.dbg')});
    system(fileRM{:});
    fileRM=strcat({'rm '},{strcat(Dir.ATPEXEDir,'\','tempLightningFile.lis')});
    system(fileRM{:});
